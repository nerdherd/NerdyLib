 ...
 */
public class SDrivetrain extends AbstractSDrivetrain {
	// Put methods for controlling this subsystem
	// here. Call these from Commands.

	protected NerdyTalon m_frontLeft, m_frontRight, m_backLeft, m_backRight;
	protected NerdyTalon m_frontLeftAngle, m_frontRightAngle, m_backLeftAngle, m_backRightAngle;
	 ...
	 */
	public SDrivetrain(int FLTalonID, int FRTalonID, int BLTalonID, int BRTalonID, 
	int FLATalonID, int FRATalonID, int BLATalonID, int BRATalonID,

	NerdyTalon frontLeft, NerdyTalon frontRight, NerdyTalon backLeft, NerdyTalon backRight,
	NerdyTalon frontLeftAngle, NerdyTalon frontRightAngle, NerdyTalon backLeftAngle, NerdyTalon backRightAngle,
	
	boolean leftInversion, boolean rightInversion) {

		m_frontLeft = new NerdyTalon(FLTalonID);
		m_frontRight = new NerdyTalon(FRTalonID);
		m_backLeft = new NerdyTalon(BLTalonID);
		m_backRight = new NerdyTalon(BRTalonID);
		m_frontLeftAngle = new NerdyTalon(FLATalonID);
		m_frontRightAngle = new NerdyTalon(FRATalonID);
		m_backLeftAngle = new NerdyTalon(BLATalonID);
		m_backRightAngle = new NerdyTalon(BRATalonID);

		// m_frontLeft.setInverted(leftInversion);
		// m_frontRight.setInverted(rightInversion);
		// m_frontLeft.setInverted(leftInversion);
		// m_frontRight.setInverted(rightInversion);
		// m_frontLeft.setInverted(leftInversion);
		// m_frontRight.setInverted(rightInversion);
		...
		m_frontLeft.configDefaultSettings();
		m_frontRight.configDefaultSettings();
		m_frontLeft.configDefaultSettings();
		m_frontRight.configDefaultSettings();
		m_frontLeftAngle.configDefaultSettings();
		m_frontRightAngle.configDefaultSettings();
		m_frontLeftAngle.configDefaultSettings();
		m_frontRightAngle.configDefaultSettings();
		...
		m_frontLeft.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative);
		m_frontRight.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative);
		m_frontLeftAngle.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative);
		m_frontRightAngle.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative);
		m_frontLeftAngle.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative);
		m_frontRightAngle.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative);
	...
	public void configNeutralMode(NeutralMode mode) {
		...
		m_frontLeftAngle.setNeutralMode(mode);
		m_frontRightAngle.setNeutralMode(mode);
		m_backLeftAngle.setNeutralMode(mode);
		m_backRightAngle.setNeutralMode(mode);
	 ...
	 * @ param continuous
	 */
	public void configCurrentLimit(int peak, int continuous, int peakA, int continuousA) {

		m_frontLeft.configCurrentLimitContinuous(continuous);
		m_frontLeft.configCurrentLimitPeak(peak);
		m_frontRight.configCurrentLimitContinuous(continuous);
		m_frontRight.configCurrentLimitPeak(peak);
		m_backLeft.configCurrentLimitContinuous(continuous);
		m_backLeft.configCurrentLimitPeak(peak);
		m_backRight.configCurrentLimitContinuous(continuous);
		m_backRight.configCurrentLimitPeak(peak);

		m_frontLeftAngle.configCurrentLimitContinuous(continuous);
		m_frontLeftAngle.configCurrentLimitPeak(peak);
		m_frontRightAngle.configCurrentLimitContinuous(continuous);
		m_frontRightAngle.configCurrentLimitPeak(peak);
		m_backLeftAngle.configCurrentLimitContinuous(continuous);
		m_backLeftAngle.configCurrentLimitPeak(peak);
		m_backRightAngle.configCurrentLimitContinuous(continuous);
		m_backRightAngle.configCurrentLimitPeak(peak);
	 ...
	 * @ param kF
	 */
	public void configFLPIDF(double kP, double kI, double kD, double kF) {
		m_frontLeft.configPIDF(kP, kI, kD, kF, 0);
	}

	public void configFRPIDF(double kP, double kI, double kD, double kF) {
		m_frontRight.configPIDF(kP, kI, kD, kF, 0);
	}

	public void configBLPIDF(double kP, double kI, double kD, double kF) {
		m_backLeft.configPIDF(kP, kI, kD, kF, 0);
	}
	
	public void configBRPIDF(double kP, double kI, double kD, double kF) {
		m_backRight.configPIDF(kP, kI, kD, kF, 0);
	}

	//angular

	public void configFLAPIDF(double kP, double kI, double kD, double kF) {
		m_frontLeftAngle.configPIDF(kP, kI, kD, kF, 0);
	}

	public void configFRAPIDF(double kP, double kI, double kD, double kF) {
		m_frontRightAngle.configPIDF(kP, kI, kD, kF, 0);
	}

	public void configBLAPIDF(double kP, double kI, double kD, double kF) {
		m_backLeftAngle.configPIDF(kP, kI, kD, kF, 0);
	}
	
	public void configBRAPIDF(double kP, double kI, double kD, double kF) {
		m_backRightAngle.configPIDF(kP, kI, kD, kF, 0);
	}
	...
	public void setSwervePower(double frontLeftPower, double frontRightPower, double backLeftPower, double backRightPower) {
		m_frontLeft.set(ControlMode.PercentOutput, frontLeftPower);
		m_frontRight.set(ControlMode.PercentOutput, frontRightPower);
		m_backLeft.set(ControlMode.PercentOutput, backLeftPower);
		m_backRight.set(ControlMode.PercentOutput, backRightPower);
	}

	public void setSwerveAnglePower(double frontLeftAnglePower, double frontRightAnglePower, double backLeftAnglePower, double backRightAnglePower) {
		m_frontLeftAngle.set(ControlMode.PercentOutput, frontLeftAnglePower);
		m_frontRightAngle.set(ControlMode.PercentOutput, frontRightAnglePower);
		m_backLeftAngle.set(ControlMode.PercentOutput, backLeftAnglePower);
		m_backRightAngle.set(ControlMode.PercentOutput, backRightAnglePower);
	}
	 ...
	 * @ param rightVoltage
	 */
	// public void setVoltage(double leftVoltage, double rightVoltage) { IMPORTANT TO IMPLEMENT -RC 12/6/21
	...
	public void resetEncoders() {
		...
		m_frontLeftAngle.setSelectedSensorPosition(0, 0, 0);
		m_frontRightAngle.setSelectedSensorPosition(0, 0, 0);
		m_backLeftAngle.setSelectedSensorPosition(0, 0, 0);
		m_backRightAngle.setSelectedSensorPosition(0, 0, 0);
	}
	...
	public void initLoggingData() {
		NerdyBadlog.createTopicStr("Drive/RightPosition", () -> String.valueOf(0));
